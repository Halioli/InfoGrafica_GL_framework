...
uniform mat4 mvpMat;
void main() {
	gl_Position = mvpMat * gl_in[0].gl_Position + vec4(0, 0, 5, 1);
	fragmentNorm = vert_Normal[0];
	fragmentPos = FragPos[0];
	fragmentUV = fragUVs[0];
	EmitVertex();
	
	gl_Position = mvpMat * gl_in[1].gl_Position + vec4(0, 0, 5, 1);
	fragmentNorm = vert_Normal[1];
	fragmentPos = FragPos[1];
	fragmentUV = fragUVs[1];
	EmitVertex();
	
	gl_Position = mvpMat * gl_in[2].gl_Position + vec4(0, 0, 5, 1);
